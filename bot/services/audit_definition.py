...
AUDIT_BLOCKS: tuple[AuditBlock, ...] = (
    ...
    AuditBlock(
        ...
        items=(
            ...
            AuditItem("4.5", "Эспрессо в рецепте, вкус, паспорт", 1),
            AuditItem("4.6", "Фильтр в рецепте, тдс, паспорт, вкус", 2),
            AuditItem("4.7", "Температура напитка соответствует норме", 2),
            AuditItem("4.8", "Еда хорошо прогрета (если требуется)", 2),
            AuditItem("4.9", "Срок годности еды в порядке", 3),
            AuditItem("4.10", "На баре порядок (вне пика)", 2),
            AuditItem("4.11", "Внешний вид бариста опрятный, форма чистая", 2),
            AuditItem("4.12", "Выдача чистая", 2),
        ),
    ),
    AuditBlock(
        index=5,
