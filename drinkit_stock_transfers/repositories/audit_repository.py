        ...
        with get_db_connection() as conn:
            try:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        "DELETE FROM audit_block_scores WHERE audit_id = %s",
                        (audit_id,),
                    )
                    execute_values(
                        cur,
                        ...
                        INSERT INTO audit_block_scores (
                            audit_id, block_index, block_title,
                            achieved, max_score, percent, comment
                        )
                        VALUES %s
                        """,
                        [
            ...
            try:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        ...
                        WHERE id = ANY(%s)
                        """,
                        (OutboxStatus.processing.value, [row[0] for row in rows]),
                    )
                conn.commit()
                return [
                    OutboxEvent(
